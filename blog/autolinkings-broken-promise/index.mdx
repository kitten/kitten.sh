...
allows for nested `node_modules` directories. This means despite conflicts, dependencies can
always be installed in nestedly. Your app may depend on `lodash@3.0.0`, but if
another dependency depends on `lodash@4.0.0`, the conflicting version is installed in a nested
`node_modules` directory. This means that while all package managers treat version ranges as
constraints, nested package managers don't outright forbid duplicated dependencies.
...
dependencies. So far so good. These being specified as peer dependencies by other dependencies
means they're installed once and we typically don't run into dedendency conflicts with them.
...
This creates an **always-implicit** dependency. For example, `react-native` depends on and
...
folder was invisible to the shallow scanning process of Expo Autolinking. With conflicts
in monorepos, the nested `node_modules` folder may actually be in your app's `node_modules`,
which was compatible with autolinking, but increased confusion due to this being inconsistent.
...
In Expo SDK 54, Expo Autolinking was completely revamped and we changes
to align it with the expectations of Node module resolution. It will now
treat Expo and React Native modules consistently. This also luckily means,
I only need to explain how it works just once. We're hoping for this to be
more intuitive and familiar than the prior implementation.
...
directories. While this search isn't active by default, when you specify any custom `searchPaths`
it still performs a shallow search, and has been kept for backwards-compatibility.
This search is also used for ["local modules"](https://docs.expo.dev/modules/native-module-tutorial/),
that are included in your app's source code. The local modules directory is specified using
`autolinking.nativeModulesDir`, and defaults to `./modules`.
...
autolink, which is more in line with regular JavaScript dependencies. (However,
for backwards compatibility, you can skip transitive React Modules by enabling
`autolinking.legacy_shallowReactNativeLinking`)
...
The Expo modules search algorithm already supported detecting and listing duplicates
before SDK 54, however, this information wasn't exposed or used properly, and
wasn't generated for React Native modules.
...
call the Metro resolver when resolving a package. Since Expo SDK 53 added
support for `package.json:exports`, you may be painfully aware of Metro's resolution
at this point.
...
they're resolve directly instead, bypassing Node resolution. This may seem weird and incorrect,
however, since this only applies to resolutions that would otherwise fail, and only activates
for a limited set of dependencies, it's typically very safe.
...
from being duplicated in your output bundle. The autolinking resolver only works for iOS and Android builds,
for now.
...
The previously mentioned Metro dependency issues apply to transformers too. In Expo, it's a custom
transformer's responsibility to call Expo's default transformers to defer to the default behaviour.
But a custom transformer from a third-party package may import different versions of
`metro-transform-worker`, `metro-babel-transformer`, or `@expo/metro-config`. We can't find
and update all third-party transformers to fix how they depend on Expo or Metro packages, but we changed
how they're loaded.
...
While this is a weird hack to apply automatically, and may lead to transformers failing if
they haven't been updated, Metro doesn't check for API output compatibility from transformers.
If Metro versions become misaligned between the Expo CLI and its transformer, the resulting
behaviour is undefined and can break your apps unexpectedly.
