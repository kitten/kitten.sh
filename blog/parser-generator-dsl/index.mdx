...
![The grammar in the draft API states that either an identifier or a string matches as often as possible, while the two are grammars of their own too.](./parser-combinators.png)
...
![A diagram of our longer grammar of the matcher as shown in the code snippet above.](./long-parse-grammar.png)
...
![Because it looks nice and fits. The graphql-parse bundle in one image. Ok, old joke thanks to Jason Miller.](./graphql-parse.webp)
...
![The RegHex Logo](./reghex-logo.png)
